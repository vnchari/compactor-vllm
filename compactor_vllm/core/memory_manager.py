...
class KVCacheManager:
    def __init__(self, rank: int, config: LLMConfig):
        ...
        # houses seq_ids that we can reclaim if needed.
        self.reclaimable_seqids = deque()
    ...
    def allocate_sequences(
        ...
        for seq_id, len_to_alloc in zip(seq_ids, max_positions):
            if seq_id not in self.seq_id_to_batch:
                ...
                if batch_id is None:
                    if len(self.reclaimable_seqids) > 0:
                        seq_id_to_discard = self.reclaimable_seqids.popleft()
                        self.free_sequences([seq_id_to_discard])
                        batch_id = self.paged_cache.new_batch()
                    else:
            ...
            if (
                self.paged_cache.reserve_tokens(
                    self.seq_id_to_batch[seq_id], len_to_alloc
                )
                != KVAllocationStatus.SUCCESS
            ):
                self.free_sequences(self.reclaimable_seqids)
                if (
    ...
    def mark_finished(self, seq_ids: Iterable[int]):
        for seq_id in seq_ids:
            self.reclaimable_seqids.append(seq_id)
    ...
    def num_free_batches(self) -> int:
        return len(self.paged_cache.free_batches) + len(self.reclaimable_seqids)
